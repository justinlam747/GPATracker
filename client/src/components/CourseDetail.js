        ...
        <div className="min-h-screen lg:h-screen lg:overflow-hidden">
            ...
            <div className="lg:hidden mobile-header-3d p-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xl font-bold text-blue-900">{course.name}</span>
                    </div>
                    <button
                        onClick={() => navigate('/')}
                        className="p-2 rounded-md text-blue-400 hover:text-blue-800"
            ...
            <div className="flex flex-col lg:flex-row lg:h-screen">
                ...
                <div className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
                    ...
                    {/* Course Info Card */}
                    <div className="card-3d-static rounded-xl p-6 mb-6">

                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                            <div>
                                <span className="text-sm font-medium text-blue-400">Course Code:</span>
                            ...
                            <div>
                                <span className="text-sm font-medium text-blue-400">Semester:</span>
                                <p className="text-lg">{course.semester} {course.year}</p>
                            </div>
                            <div>
                                <span className="text-sm font-medium text-blue-400">GPA Scale:</span>
                                <p className="text-lg">
                        ...
                        {/* Final Grade Display */}
                        <div className="insight-display rounded-xl p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-medium text-blue-900">Final Grade</h3>
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-blue-400"
                                >
                    ...
                    <div className="card-3d-static rounded-xl p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-2xl font-bold text-blue-900">Mark Breakdown</h2>
                            <button
                                onClick={() => setShowAssignmentForm(true)}
                                className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md btn-3d-secondary text-blue-900"
                            >
                        ...
                        {error && (
                            <div className="alert-error-3d px-4 py-3 rounded-md mb-4">
                                {error}
                            </div>
                        )}

                        {success && (
                            <div className="alert-success-3d px-4 py-3 rounded-md mb-4">
                                {success}
                            </div>
                        )}
                        ...
                        {course.assignments && course.assignments.length > 0 ? (
                            <div className="space-y-4">
                                {course.assignments.map((assignment) => (
                                    <div key={assignment.id} className="checklist-row rounded-xl p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center space-x-3">
                                                <h3 className="text-lg font-medium text-blue-900">{assignment.name}</h3>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                                            ...
                                            <div>
                                                <span className="font-medium text-blue-400">Grade:</span>
                                                <p className="font-medium">{assignment.grade}</p>
                                            </div>
                                            <div>
                                                <span className="font-medium text-blue-400">Max Grade:</span>
                                                <p>{assignment.maxGrade}</p>
                                            </div>
                                            <div>
                                                <span className="font-medium text-blue-400">Due Date:</span>
                                        ...
                                        {assignment.notes && (
                                            <div className="mt-3">
                                                <span className="font-medium text-blue-400">Notes:</span>
                                                <p className="text-sm text-blue-400 mt-1">{assignment.notes}</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
                    ...
                    {course.assignments && course.assignments.length > 0 && (
                        <div className="card-3d-static rounded-xl p-6 mt-6">
                            <h2 className="text-2xl font-bold text-blue-900 mb-6">Assignment Performance</h2>

                            {/* Performance Summary */}
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="insight-display rounded-xl p-4">
                                    <h4 className="text-sm font-medium text-blue-400">Average Grade</h4>
                                ...
                                <div className="insight-display rounded-xl p-4">
                                    <h4 className="text-sm font-medium text-blue-400">Total Weight</h4>
                                    <p className="text-2xl font-bold text-blue-600">
                                        {course.assignments.reduce((sum, assignment) => sum + (assignment.weight || 0), 0)}%
                                    </p>
                                </div>
                                <div className="insight-display rounded-xl p-4">
                                    <h4 className="text-sm font-medium text-blue-400">Assignments</h4>
                                    <p className="text-2xl font-bold text-blue-600">
                    ...
                    {showAssignmentForm && (
                        <div className="fixed inset-0 modal-overlay-3d overflow-y-auto h-full w-full z-50">
                            <div className="relative top-20 mx-auto p-5 card-3d-static w-96 rounded-2xl">
                                ...
                                <form onSubmit={editingAssignment ? handleUpdateAssignment : handleAddAssignment} className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            required
                                            className="mt-1 block w-full input-3d px-3 py-2"
                                        />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-blue-900">Weight (%) *</label>
                                            <input
                                                type="number"
                                                name="weight"
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-blue-900">Max Grade</label>
                                            <input
                                                type="number"
                                                name="maxGrade"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-blue-900">Grade *</label>
                                        <input
                                            type="text"
                                            name="grade"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-blue-900">Due Date</label>
                                        <input
                                            type="date"
                                            name="dueDate"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-blue-900">Notes</label>
                                        <textarea
                                            name="notes"
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        />
                                        <label className="ml-2 block text-sm text-blue-900">Mark as completed</label>
                                    </div>
                                    ...
                                    <div className="flex justify-between space-x-3">
                                        <button
                                            type="button"
                                            onClick={resetAssignmentForm}
                                            className="px-4 py-2 rounded-md text-sm font-medium btn-3d-secondary"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            type="submit"
                                            className="px-4 py-2 rounded-md text-sm font-medium btn-3d-primary text-white"
                                        >
                    ...
                    {showGradeOverride && (
                        <div className="fixed inset-0 modal-overlay-3d overflow-y-auto h-full w-full z-50">
                            <div className="relative top-20 mx-auto p-5 card-3d-static w-96 rounded-2xl">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-medium text-blue-900">Set Grade Override</h3>
                                    <button
                                ...
                                <form onSubmit={handleGradeOverride} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-blue-900 mb-2">Grade Type</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-blue-900">Grade Override *</label>
